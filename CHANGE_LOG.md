* 2026.04.17 (dev): cap ETKDGv3 `maxIterations` to prevent hangs on difficult ring systems; add `MMFF` conformation as an option; fix spectrum normalization to be per-spectrum rather than per-batch; fix missing mask key in augmented data entries; move training hyperparameters to configuration files; remove redundant `batch_size` from model config sections.
